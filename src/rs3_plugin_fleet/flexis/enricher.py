...
def _upsert_series(df: pd.DataFrame, target_snake: str, values: pd.Series) -> None:
    ...
    if existing and existing != target_snake and existing in df.columns:
        try: df.drop(columns=[existing], inplace=True)
...
_ALIAS_TO_CANON = {
    ...
    "FlexisRoadInfrastrcture": "flexis_infra_event",
    "FlexisNightCondition": "flexis_night",
    "FlexisDrivingBeheviour": "flexis_driver_event",
    "FlexiTraffic": "flexis_traffic_level",
    "FlexiWetaherCondition": "flexis_weather",
    "Fexis Delivery Status": "flexis_delivery_status",
    "FlexiSpulationDensity": "flexis_population_density_km2",
}
...
def _label_from_schedule(mins: pd.Series, schedule: Any, key: str) -> pd.Series:
    """Accepte list/dict/set/DataFrame — produit labels par fenêtres [from,to)."""
    # normalisation
    if schedule is None:
    ...
    if isinstance(schedule, (tuple, set)):
        ...
        try:
            d = schedule.to_dict(orient="records")  # type: ignore
            schedule = d
        except Exception:
            schedule = [schedule]
...
def _delta_heading(h):
    d = np.diff(h, prepend=h[0]); d = (d + 180.0) % 360.0 - 180.0
    return d
...
def _osm_heuristic_density(road_type: pd.Series) -> pd.Series:
    table = {"residential": 3000,"tertiary": 2000,"secondary": 1500,"primary": 800,"service": 1200,"trunk": 400,"motorway": 200}
    s = road_type.astype(str) if isinstance(road_type, pd.Series) else pd.Series("", index=getattr(road_type, "index", None))
...
class FlexisEnricher:
    ...
    def apply(self, df: pd.DataFrame, context: Dict[str, Any] | None = None) -> pd.DataFrame:
        df = df.copy(); context = context or {}

        # altitude
        altitude = None
        for c in ("altitude", "elev_m", "elevation"):
            if c in df.columns:
                altitude = pd.to_numeric(df[c], errors="coerce"); break
        df["altitude"] = altitude if altitude is not None else np.nan

        # road_type
        if "road_type" in df.columns:
            _upsert_series(df, "flexis_road_type", df["road_type"].fillna("unknown").astype(str))
        ...
        # nav events
        nav = self._nav_events(df); _upsert_series(df, "flexis_nav_event", nav)

        # minutes de la journée
        if "timestamp" in df.columns:
            mins = _tod_minutes(df["timestamp"])
        else:
            mins = pd.Series(0, index=df.index)

        # météo / nuit / trafic (robustes à dict/DataFrame)
        wt = self.cfg.weather_timeline
        weather = _label_from_schedule(mins, wt, key="weather") if wt is not None else pd.Series("", index=df.index)
        ...
        traffic = _label_from_schedule(mins, tp, key="level") if tp is not None else pd.Series("", index=df.index)
        _upsert_series(df, "flexis_traffic_level", traffic)

        # statut livraison
        _upsert_series(df, "flexis_delivery_status", self._delivery_timeline(df, context))
        # infra / driver
    ...
    def _driver_events(self, df: pd.DataFrame) -> pd.Series:
        ...
        dv = v.diff(); dt = pd.Series(t).diff().replace(0, np.nan)
        a = dv / dt
